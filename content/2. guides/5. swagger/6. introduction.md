...
description: Learn how @charcole/swagger eliminates schema duplication and makes API documentation effortless.
navigation:
  icon: i-lucide-book-open
seo:
  title: "Charcole Swagger Documentation"
  description: Learn how @charcole/swagger auto-generates OpenAPI documentation from Zod schemas with zero duplication.
  keywords:
    - charcole swagger
...
**`@charcole/swagger`** exists to end this cycle.
...
`@charcole/swagger` removes the duplication entirely. Schemas are defined once in Zod. Documentation is generated automatically from that single source of truth.

This is not just convenient. It is structurally impossible for documentation to drift.

---

## What `@charcole/swagger` actually does
...
## The scale of the problem `@charcole/swagger` solves

Before `@charcole/swagger`, a typical authenticated endpoint required around **76 lines of Swagger documentation**.
...
With `@charcole/swagger`, those same 20 endpoints require **400 lines total**.
...
`@charcole/swagger` includes six common response templates that work out of the box:
...
`@charcole/swagger` removes the tradeoff entirely.
...
`@charcole/swagger` does not lock you into anything.

It works with any Express.js project. You can install it via `npm install @charcole/swagger` and use it even if you never used Charcole itself.
...
The real benefit of `@charcole/swagger` is not immediate.
...
`@charcole/swagger` does exactly that.

---

## Final thoughts

`@charcole/swagger` is not fancy.
